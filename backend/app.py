...
# --- НОВИЙ КОД: Імпорти для ШІ та .env ---
...
# --- НОВИЙ КОД: Завантаження .env та конфігурація ШІ ---
load_dotenv()
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")
if GOOGLE_API_KEY:
    genai.configure(api_key=GOOGLE_API_KEY)
else:
    print("WARNING: GOOGLE_API_KEY не знайдено. Інтерпретація ШІ буде вимкнена.")
# --- КІНЕЦЬ НОВОГО КОДУ ---


# ... (ВЕСЬ ВАШ ІСНУЮЧИЙ КОД КОНФІГУРАЦІЇ ДО /generate) ...
# (ZODIAC_SYMBOLS, PLANET_SYMBOLS, ASPECTS_DEF, і всі функції-утиліти залишаються без змін)
...
def draw_natal_chart(chart, aspects_list, save_path, name_for_center=None,
    ...
    """
    Малює натальну карту:
      - сектори домів + номери
      - кільце зодіаку з підписами (символи + назви)
      - ASC/MC/DSC/IC маркери з градусами
      - планети (символи + градуси)
      - хорди аспектів (кольори з ASPECTS_DEF)
      - легенда, центр-логотип
    Вхід:
      chart - flatlib.chart.Chart
      aspects_list - список словників із keys: planet1, planet2, type, color, angle, angle_dms
      save_path - куди зберегти PNG
    """
    try:
        fig = plt.figure(figsize=(12, 12))
        ax = plt.subplot(111, polar=True)

        # орієнтація: 0 = West (ліва сторона), CCW (щоб зодіак йшов CCW від ASC)
        ...
        # -- helper: безпечне отримання об'єкта з chart --
        ...
        # локальна конвертація довготи -> радіани від ASC (0..360 -> rad)
        def to_theta_from_asc(lon):
            ang = (float(lon) - float(asc_lon)) % 360.0
            return np.deg2rad(ang)

        # глобальна (для знаків/кілець) — також від ASC, але приймає degree
        def to_theta_global(deg):
            return to_theta_from_asc(deg)
        ...
        for i in range(1, 13):
            ...
            ax.text(to_theta_global(mid), house_number_radius, str(i),
        ...
        # ---------- 7) Планети: точки + символи + градуси ----------
        r_planet = 0.85
        planet_positions = {}
        ...
        def deg_in_sign_text(lon):
            sign = int(lon // 30)
            deg = lon % 30
            return f"{int(deg)}° {SIGN_SYMBOLS_STR[sign]}"

        # збираємо планети у planet_positions
        for pid in PLANETS + ["Ascendant","MC","IC","DSC"]:
            ...
            ax.text(th, r_label, deg_in_sign_text(lon), fontsize=8, ha="center", va="center", color=col,
        ...
        ax.legend(handles=legend_elements, loc="upper center", bbox_to_anchor=(0.5, -0.19),
...
def generate():
    try:
        ...
        try:
            if os.path.exists(json_path) and os.path.exists(png_path):
                ...
                if dt.now() - mtime <= timedelta(days=CACHE_TTL_DAYS):
                    ...
                    # --- ЗМІНА: перевіряємо, чи є в кеші інтерпретація
                    if cached.get("ai_interpretation"):
        ...
        try:
            for obj in chart.objects:
                ...
                if pid in PLANET_SYMBOLS:
                    # Додаємо більше деталей для ШІ
                    sign_idx, deg_in_sign = zodiac_position(obj.lon)
                    planets_list.append({
                        "name": pid,
                        ...
                        "house": obj.house,
                        "is_retro": getattr(obj, "retro", False)
                    })
        except Exception:
            planets_list = []
        ...
        # --- НОВИЙ КОД: Генерація інтерпретації ШІ ---
        ai_interpretation = None
        if GOOGLE_API_KEY:
            try:
                model = genai.GenerativeModel('gemini-1.5-flash')
                
                # Створюємо детальний промпт
                prompt = f"""
                Ти — професійний, мудрий та емпатичний астролог на ім'я Дарія Альбірео.
                Твоя задача — написати глибоку, але зрозумілу інтерпретацію натальної карти для людини на ім'я {name}.
                Звертайся до людини на "ти". Твій стиль — підтримуючий та надихаючий.

                Проаналізуй наступні астрологічні дані, надані у форматі JSON.
                Напиши зв'язний, цілісний текст, а не просто перелік характеристик.
                Використовуй Markdown для форматування (заголовки, списки, жирний шрифт).

                **Структура аналізу:**

                1.  **Привітання та загальне ядро особистості:**
                    * Коротко привітайся з {name}.
                    * Опиши ключову енергію, базуючись на положенні Сонця, Місяця та Асцендента.

                2.  **Сильні сторони та таланти:**
                    * Визнач найпотужніші та найгармонійніші аспекти (тригони, секстилі) та планети.
                    * Розкажи, в яких сферах життя (будинках) ці таланти можуть проявитися найяскравіше.

                3.  **Внутрішні виклики та точки росту:**
                    * Делікатно опиши напружені аспекти (квадратури, опозиції).
                    * Поясни це не як "проблеми", а як "зони розвитку" та внутрішні конфлікти, які ведуть до зростання.
                    * Дай пораду, як можна гармонізувати ці енергії.

                4.  **Сфера стосунків та кохання:**
                    * Проаналізуй положення Венери та Марса, а також 7-й будинок.

                5.  **Кар'єра та соціальна реалізація:**
                    * Проаналізуй MC (10-й будинок), положення Сатурна та Юпітера.

                **Астрологічні дані для аналізу:**
                ...
                print(f"Помилка при генерації тексту ШІ: {e}")
                ai_interpretation = "На жаль, не вдалося згенерувати інтерпретацію. Спробуйте пізніше."
        
        out["ai_interpretation"] = ai_interpretation
        # --- КІНЕЦЬ НОВОГО КОДУ ---
